...
export default function AdminPanel() {
  ...
  function moveImageUrl(index: number, direction: -1 | 1) {
    const newArr = [...form.image_urls];
    const newIndex = index + direction;
    if (newIndex < 0 || newIndex >= newArr.length) return;
    [newArr[index], newArr[newIndex]] = [newArr[newIndex], newArr[index]];
    setForm((prev) => ({ ...prev, image_urls: newArr }));
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={openAdd}
            className="bg-[#F5A623] text-white text-sm font-bold px-4 py-2 rounded-lg hover:bg-[#d8921f] transition-colors"
          >
            + 상품 추가
          </button>
          <form action={logout}>
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          <div className="relative flex-1 max-w-xs">
            <input
              ...
              placeholder="제목 검색..."
              className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm outline-none focus:border-[#F5A623] transition-colors pr-7"
            />
            {searchQuery && (
              <button
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4 overflow-y-auto">
              ...
              <div>
                ...
                {imageInputMode === "url" ? (
                  ...
                  form.image_url ? (
                    ...
                    <label className="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-gray-200 rounded-lg cursor-pointer hover:border-[#F5A623] transition-colors">
                      {uploading ? (
              ...
              <div>
                ...
                {form.video_url ? (
                  ...
                  <label className="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-gray-200 rounded-lg cursor-pointer hover:border-[#F5A623] transition-colors">
                    {uploadingVideo ? (
                      <span className="text-sm text-gray-400">업로드 중...</span>
                    ) : (
                      <>
                ...
                {form.image_urls.length > 0 && (
                  <div className="flex flex-col gap-1.5">
                    {form.image_urls.map((url, i) => (
                      <div key={i} className="flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-lg px-2 py-1.5">
                        {brokenImages.has(url) ? (
                          <div className="w-10 h-10 rounded flex-shrink-0 bg-gray-100 flex items-center justify-center text-sm">❌</div>
                        ) : (
                          <img
                            src={url}
                            alt=""
                            className="w-10 h-10 rounded object-cover flex-shrink-0 bg-gray-200"
                            onError={() => setBrokenImages((prev) => new Set([...prev, url]))}
                          />
                        )}
                        <span className="flex-1 text-xs text-gray-500 truncate min-w-0">{url}</span>
                        <div className="flex gap-1 flex-shrink-0">
                          <button
                            type="button"
                            onClick={() => moveImageUrl(i, -1)}
                            disabled={i === 0}
                            className="w-7 h-7 flex items-center justify-center rounded text-gray-400 hover:text-gray-700 hover:bg-gray-200 transition-colors disabled:opacity-30"
                          >
                            ↑
                          </button>
                          <button
                          ...
                          <button
                            ...
                            className="w-7 h-7 flex items-center justify-center rounded text-red-300 hover:text-red-500 hover:bg-red-50 transition-colors"
                          >
                            🗑️
                          </button>
                        </div>
